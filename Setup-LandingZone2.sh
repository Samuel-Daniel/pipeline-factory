...
aws cloudformation deploy --stack-name RoleCrossAccount --template-file General/Setup-CrossAccount.yaml --capabilities CAPABILITY_NAMED_IAM \
--parameter-overrides DevOpsAccount=$DevOpsAccount DevAccount=$DevAccount HomologAccount=$HomologAccount ProdAccount=$ProdAccount  --profile DevAccount \
&
aws cloudformation deploy --stack-name RoleCrossAccount --template-file General/Setup-CrossAccount.yaml --capabilities CAPABILITY_NAMED_IAM \
...
aws cloudformation deploy --stack-name RoleCrossAccount --template-file General/Setup-Roles.yaml --capabilities CAPABILITY_NAMED_IAM \
--parameter-overrides DevOpsAccount=$DevOpsAccount DevAccount=$DevAccount HomologAccount=$HomologAccount ProdAccount=$ProdAccount DevToolsAccount=$DevToolsAccount  --profile DevToolsAccount
